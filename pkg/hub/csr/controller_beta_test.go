...
func Test_isV1beta1SpokeClusterClientCertRenewal(t *testing.T) {
	invalidSignerName := "invalidsigner"

	tests := []struct {
		name      string
		csrHolder testinghelpers.CSRHolder
		isRenewal bool
	}{
		{
		...
		{
			name: "a renewal csr without singer name",
			csrHolder: testinghelpers.CSRHolder{
				Name:         validV1beta1CSR.Name,
				Labels:       validV1beta1CSR.Labels,
				SignerName:   "",
		...
		{
			name:      "a renewal csr",
			csrHolder: validV1beta1CSR,
			isRenewal: true,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := isV1beta1SpokeClusterClientCertRenewal(testinghelpers.NewV1beta1CSR(tt.csrHolder)); got != tt.isRenewal {
				t.Errorf("isV1beta1SpokeClusterClientCertRenewal() = %v, want %v", got, tt.isRenewal)
			}
		})
	}
}
