...
func shouldCreateCSR(
	...
	default:
		...
		logger.V(4).Info("Client certificate for:","name",controllerName,"time total",total,"remaining", remaining, "remaining/total",remaining.Seconds()/total.Seconds())
		...
		if remaining.Seconds()/total.Seconds() > threshold {
			...
			logger.V(4).Info("Client certificate validity and life remaining", "controller", controllerName, "lifeRemainingPercentage", threshold*100)
			return false, nil
		}
