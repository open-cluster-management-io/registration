...
var map_ManagedClusterLabel = map[string]string{
	"":      "ManagedClusterLabel defines one label of managedCluster",
	"key":   "The key must be cluster.open-cluster-management.io/clusterset, it will be set by default if unspecified",
	"value": "The value must be <clusterSet name>, it will be set by default if unspecified",
}

func (ManagedClusterLabel) SwaggerDoc() map[string]string {
	return map_ManagedClusterLabel
}

var map_ManagedClusterSelector = map[string]string{
	"":               "ManagedClusterSelector represents a selector of ManagedClusters",
	"selectorType":   "SelectorType could only be ExclusiveLabel now, will support more SelectorType later \"ExclusiveLabel\" means to use a particular cluster label. It guarantees that clustersets with same label key are exclusive",
	"exclusiveLabel": "ExclusiveLabel defines one label which clusterset could use to select target managedClusters. In this way, we will: 1. Guarantee clustersets with same label key are exclusive 2. Enable additional permission check when cluster joining/leaving a clusterset. Currentlly, When a user want to add a label \"cluster.open-cluster-management.io/clusterset:<ManagedClusterSet Name>\" for managedCluster, he/she should have \"create\" permission for subresource \"managedclusters/label\" with name \"cluster.open-cluster-management.io/clusterset:<ManagedClusterSet Name>\".",
}
